...
(defun chun-blog/blog-template (title content &optional description)
  ...
  (concat
   ...
   "      <a class=\"navbar-item\" href=\"" chun-blog/publish-url "\">"
   "         Superjomn's Blog"
   "      </a>"
   "    </div>"
   ...
   "        <a class=\"navbar-item\" href=\"" chun-blog/publish-url  "\">\n"
              "Home\n"
   "        </a>"
   "        <a class=\"navbar-item\" href=\"" (concat-to-dir chun-blog/publish-url "archive.html")  "\">\n"
   "          Archive\n"
   "        </a>"
   "        <a class=\"navbar-item\" href=\"" chun-blog/publish-url  "\">\n"
   "        About\n"
...
e.g. input 'hello.org' get '~/blog/public/hello.html'. "
  (let* ((basename (file-name-sans-extension post-name))
         (html-path (concat-to-dir chun-blog/publish-directory (concat basename ".html"))))
    html-path))
...
Depends on chun-blog/langcode and chun-blog/texts."
  (let* ((text-node (assoc text-id chun-blog/texts))
    ...
    (if text-lang-node
	(cdr text-lang-node)
      (concat "[" (symbol-name text-id) ":" chun-blog/langcode "]"))))
...
This function is called for every post on the archive and tags-archive page."
  (concat
   "<div class=\"post-date\">"
   (format-time-string (chun-blog/gettext 'date-format) (chun-blog/get-date post-filename))
   "</div>"
   "<h2 class=\"post-title\">"
   "<a href=\"" (chun-blog/get-post-url post-filename) "\">" (chun-blog/get-title post-filename) "</a>"
   "</h2>\n")
  )
...
(org-export-define-derived-backend 'chun-blog/post-bare 'html
...
(defun chun-blog/file-buffer (filepath)
  ...
  (car (seq-filter
        (lambda (buf)
          (string= (with-current-buffer buf buffer-file-name) filepath))
        (buffer-list))))
      ...
      (let ((current-buffer (current-buffer))
        ...
        (with-temp-buffer
          (if buffer-exists
            ...
            (insert-file-contents post-filename))
          (chun--info "file-content: %s" (buffer-substring (point-min) (point-max)))
          (org-mode)
          (goto-char (point-min))
          ...
          (chun--info "file-content2: %s" (buffer-substring (point-min) (point-max)))
          (setq result
                (org-export-as 'chun-blog/post-bare nil nil nil nil))
...
The index, archive, tags, and RSS feed are not updated. "
  (interactive "f")
  (chun-blog/with-find-file
   (chun-blog/matching-publish-filename post-filename)
   (chun-blog/blog-template
...
The index page contains the last `chun-blog/index-length' posts as full text posts. "
  (let ((post-filenames (chun-blog/get-post-filenames)))
    ...
    (setq post-filenames (sort post-filenames (lambda (x y) (time-less-p (chun-blog/get-date x)
                                                                         (chun-blog/get-date y)))))
    (chun-blog/assemble-multipost-page
     (concat-to-dir chun-blog/publish-directory "index.html")
...
blog post, but no post body."
  (let ((archive-filename (concat-to-dir chun-blog/publish-directory chun-blog/archive-file))
        (archive-entries nil)
    ...
    (chun-blog/with-find-file
     archive-filename
     (chun-blog/blog-template
      chun-blog/publish-title
      (concat
       "<h1 class=\"title\">" (chun-blog/gettext 'archive) "</h1>\n"
       (apply 'concat (mapcar 'chun-blog/get-post-summary post-filenames)))))))
...
(defun chun-blog/assemble-multipost-page (pub-filename post-filenames &optional front-matter)
  ...
  (setq post-filenames (sort post-filenames (lambda (x y) (time-less-p (chun-blog/get-date y)
                                                                       (chun-blog/get-date x)))))
  (chun-blog/with-find-file
   pub-filename
   (chun-blog/blog-template
    ...
    (concat
     ...
     (apply 'concat (mapcar (if chun-blog/use-preview 'chun-blog/get-preview
                              'chun-blog/get-body) post-filenames))
     "<div id=\"archive\">\n"
  ...
  (with-temp-buffer
    ...
    (let* ((post-title (chun-blog/get-title post-filename))
      ...
      (when (and preview-region (search-forward "<p>" nil t))
        (setq post-ellipsis
              (concat (when chun-blog/preview-link-p
                        (format "<a href=\"%s\">"
      ...
      (let ((title-link
             (format "<h2 class=\"post-title\"><a href=\"%s\">%s</a></h2>"
                     (chun-blog/get-post-url post-filename) post-title))
            (date-link
...
Only blog posts that changed since the HTML was created are re-rendered. "
  ...
  (dolist (file (append (chun-blog/get-post-filenames)
    ...
    (when (or force-render (chun-blog/needs-publishing-p file))
